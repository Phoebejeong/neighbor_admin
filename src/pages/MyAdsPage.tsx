    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">기간 한정 이벤트/프로모션을 입주민에게 홍보하세요 (유료)</p>
        </div>
      ...
      {shops.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p className="text-sm text-stone-400 mt-1">광고를 신청하려면 먼저 가게를 등록해주세요</p>
      ...
      {ads.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p className="text-stone-500 font-medium">등록한 광고가 없습니다</p>
          <p className="text-sm text-stone-400 mt-1">알짜광고를 등록해서 입주민에게 이벤트를 알려보세요</p>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center justify-between px-5 pt-5 pb-3">
                      <div className="flex items-center gap-2 min-w-0">
                        <h3 className="text-lg font-bold text-stone-900 truncate">{ad.title}</h3>
                        <span className={`text-xs font-semibold shrink-0 ${st.cls}`}>{st.label}</span>
                      </div>
                      <div className="flex items-center gap-1 ml-4 shrink-0">
                        <button onClick={() => openEdit(ad)} className="p-2 rounded-lg hover:bg-[#FDF2F2] text-stone-400 hover:text-[#7f2929] transition"><SquarePen className="w-5 h-5" /></button>
                        <button onClick={() => remove(ad.id)} className="p-2 rounded-lg hover:bg-red-100 text-stone-400 hover:text-red-500 transition"><Trash2 className="w-5 h-5" /></button>
                      </div>
                    </div>
                    {/* 설명 */}
                    <p className="text-sm text-stone-600 px-5 pb-3 line-clamp-2">{ad.description}</p>
                    {/* 중간: 기간 / 조회수 / 금액 3칸 그리드 */}
                    <div className="grid grid-cols-3 border-t border-stone-100 divide-x divide-stone-100">
                      <div className="px-5 py-3">
                        <p className="text-sm text-stone-400 font-medium mb-1">광고 기간</p>
                        <p className="text-sm font-bold text-stone-800">{ad.period || '-'}</p>
                      </div>
                      <div className="px-5 py-3">
                        <p className="text-sm text-stone-400 font-medium mb-1">조회수</p>
                        <p className="text-sm font-bold text-stone-800">{formatMoney(ad.viewCount)}회</p>
                      </div>
                      <div className="px-5 py-3">
                        <p className="text-sm text-stone-400 font-medium mb-1">광고비</p>
                        <p className="text-sm font-bold text-[#7f2929]">{formatMoney(ad.amount)}원</p>
                      </div>
                    </div>
                    {/* 하단: 아파트 */}
                    {aptCount > 0 && (
      ...
      <Modal open={open} title={step === 'confirm' ? '신청 확인' : step === 'apartments' ? '홍보 대상 아파트' : (editing?.id ? '광고 수정' : '알짜광고 신청')} onClose={close}>
        ...
        {editing && step === 'form' && (
          <div className="space-y-4">
            ...
            {shops.length > 0 && (
              <div>
                <label className="block text-xs font-semibold text-stone-500 mb-1.5">광고 가게 선택</label>
                <select
            ...
            <div>
              <label className="block text-xs font-semibold text-stone-500 mb-1.5">광고 제목 *</label>
              <input value={editing.title} onChange={e => { setEditing({ ...editing, title: e.target.value }); setErrors({ ...errors, title: '' }); }} placeholder="예: 봄맞이 할인 이벤트" className={`w-full border rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:outline-none ${errors.title ? 'border-red-300 focus:ring-red-500' : 'border-stone-200 focus:ring-[#7f2929]'}`} />
            ...
            <div>
              <label className="block text-xs font-semibold text-stone-500 mb-1.5">광고 내용 *</label>
              <textarea value={editing.description} onChange={e => { setEditing({ ...editing, description: e.target.value }); setErrors({ ...errors, description: '' }); }} rows={3} placeholder="이벤트 상세 내용을 작성해주세요" className={`w-full border rounded-lg px-3 py-2.5 text-sm resize-none focus:ring-2 focus:outline-none ${errors.description ? 'border-red-300 focus:ring-red-500' : 'border-stone-200 focus:ring-[#7f2929]'}`} />
            ...
            <div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-stone-500 mb-1.5">시작일 *</label>
                  <input type="date" value={editing.startDate} min={new Date().toISOString().slice(0, 10)} onChange={e => { setEditing({ ...editing, startDate: e.target.value, endDate: editing.endDate && editing.endDate < e.target.value ? '' : editing.endDate }); setErrors({ ...errors, startDate: '' }); }} className={`w-full border rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:outline-none ${errors.startDate ? 'border-red-300 focus:ring-red-500' : 'border-stone-200 focus:ring-[#7f2929]'}`} />
                ...
                <div>
                  <label className="block text-xs font-semibold text-stone-500 mb-1.5">종료일 *</label>
                  <input type="date" value={editing.endDate} min={editing.startDate || new Date().toISOString().slice(0, 10)} onChange={e => { setEditing({ ...editing, endDate: e.target.value }); setErrors({ ...errors, endDate: '' }); }} className={`w-full border rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:outline-none ${errors.endDate ? 'border-red-300 focus:ring-red-500' : 'border-stone-200 focus:ring-[#7f2929]'}`} />
              ...
              {(() => {
                const days = calcDays(editing.startDate, editing.endDate);
                return days > 0 ? (
                  <p className="text-xs font-semibold text-[#7f2929] mt-2">{days}일 선택됨</p>
                ) : null;
              })()}
            </div>
            ...
            <div className="border-t border-stone-100 pt-4">
              <p className="text-xs font-semibold text-stone-400 mb-2">앱 미리보기</p>
              <AdPreviewCard
                title={editing.title}
          ...
          return (
            <div className="space-y-4">
              ...
              <div className="bg-[#FDF2F2] rounded-lg p-5">
                <h4 className="text-sm font-bold text-[#5C1A1A] mb-3">신청 내용 확인</h4>
              ...
              <div className="bg-stone-900 rounded-lg p-5 text-white">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex items-center justify-between text-lg font-bold pt-1">
                    <span>총 금액</span>
                    <span className="text-[#A05050]">{formatMoney(totalAmount)}원</span>
                  </div>
                </div>
              </div>
