    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">등록된 가게를 관리하고 입주민 앱에 노출하세요 ({shops.length}/{MAX_SHOPS})</p>
        </div>
      ...
      {shops.length === 0 ? (
        ...
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p className="text-sm text-stone-400 mt-1">등록하시면 입주민 앱에 무료로 노출됩니다</p>
          <button onClick={openCreate} className="mt-4 bg-[#222] hover:bg-[#333] text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition">
            가게 등록하기
          </button>
        </div>
      ) : (
              ...
              <div key={shop.id} className="bg-white rounded-lg shadow-sm overflow-hidden">
                ...
                <div className="px-4 py-3 flex items-center justify-between">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="w-9 h-9 rounded-lg bg-[#FDF2F2] flex items-center justify-center text-sm font-bold text-[#7f2929] shrink-0">
                      {shop.name.charAt(0)}
                    </div>
                    <div className="min-w-0">
                      <h3 className="text-sm font-bold text-stone-900 truncate">{shop.name}</h3>
                      <p className="text-xs text-stone-400">{shop.category}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`text-xs font-semibold ${shop.isHomeVisible ? 'text-emerald-500' : 'text-stone-400'}`}>
                      {shop.isHomeVisible ? '노출 중' : '비노출'}
                    </span>
                    <button onClick={() => openEdit(shop)} className="p-1.5 rounded-lg hover:bg-[#FDF2F2] text-stone-400 hover:text-[#7f2929] transition">
                      <SquarePen className="w-4 h-4" />
                    </button>
                    <button onClick={() => deleteShop(shop.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-stone-400 hover:text-red-500 transition">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                </div>
                {/* 정보 */}
                <div className="px-4 py-2 border-t border-stone-100 text-xs text-stone-600">
                ...
                {/* 아파트 */}
                {aptCount > 0 && (
                  <div className="flex items-center gap-2 px-4 py-2 border-t border-stone-100 bg-stone-50/50">
                    <Building2 className="w-3.5 h-3.5 text-stone-400 shrink-0" />
                    <span className="text-xs font-semibold text-stone-500 shrink-0">총 {aptCount}개</span>
                    <span className="text-stone-300">|</span>
                    <div className="flex items-center gap-1 flex-wrap">
                      {aptNames.slice(0, 3).map((n, i) => (
                        <span key={i} className="text-xs px-1.5 py-0.5 bg-white rounded text-stone-500 border border-stone-200">{n}</span>
                      ))}
                      {aptNames.length > 3 && <span className="text-xs text-stone-400">+{aptNames.length - 3}</span>}
                    </div>
                  </div>
                )}
              </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-lg shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            {/* 헤더 */}
            <div className="sticky top-0 bg-white border-b border-stone-100 px-6 py-4 flex items-center justify-between z-10 rounded-t-lg">
              <div>
              ...
              <button onClick={closeModal} className="w-8 h-8 rounded-lg hover:bg-stone-100 flex items-center justify-center transition">
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value, icon: ICONS[e.target.value] || 'store-outline' })} className="w-full border border-stone-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-[#7f2929] focus:outline-none">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm text-stone-600">홈 화면에 내 가게 노출하기</span>
              </label>

              {/* ── 아파트 선택 ── */}
              <div>
                <ApartmentSelector
                  selected={form.targetApartments}
                  onChange={ids => setForm(prev => ({ ...prev, targetApartments: ids }))}
                />
              ...
              <div className="bg-stone-50 rounded-lg p-4">
                <p className="text-xs text-stone-500">
                  이웃상가 등록은 <strong className="text-[#7f2929]">무료</strong>입니다. 등록하시면 선택한 아파트 입주민 앱의 "우리동네 &gt; 이웃상가" 탭에 노출됩니다.
                </p>
              </div>
            </div>

            {/* 하단 버튼 */}
            <div className="sticky bottom-0 bg-white border-t border-stone-100 px-6 py-4 flex gap-3 rounded-b-lg">
              <button onClick={closeModal} className="flex-1 py-3 rounded-lg border border-stone-200 text-sm font-medium text-stone-500 hover:bg-stone-50 transition">
                취소
              </button>
...
const InfoRow: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div className="flex items-center justify-between py-1.5 border-b border-stone-100 last:border-0">
    ...
    <span className="text-xs font-medium text-stone-900 text-right max-w-[60%] truncate">{value}</span>
  </div>
);
...
const Field: React.FC<{ label: string; value: string; onChange: (v: string) => void; placeholder?: string; error?: string }> = ({ label, value, onChange, placeholder, error }) => (
  <div>
    ...
    <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className={`w-full border rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:outline-none ${error ? 'border-red-300 focus:ring-red-500' : 'border-stone-200 focus:ring-[#7f2929]'}`} />
