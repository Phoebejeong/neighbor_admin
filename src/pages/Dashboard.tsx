  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-stone-500 mt-1">우리동네 서비스 현황을 한눈에 확인하세요</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-gradient-to-br from-[#7f2929] to-[#6B2222] rounded-xl p-5 text-white hover:shadow-md transition">
          <p className="text-xs font-medium text-[#A05050] uppercase tracking-wider">이번 달 수익</p>
          <p className="text-3xl font-extrabold mt-1">{formatMoney(totalRevenue)}<span className="text-base font-medium text-[#A05050] ml-1">원</span></p>
          <p className="text-xs text-[#A05050] mt-2">광고 {formatMoney(adRevenue)}원 + 쇼핑 {formatMoney(shopRevenue)}원</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-stone-200 p-5">
          <h3 className="text-sm font-bold text-stone-700 mb-3">조회수 TOP 5 광고</h3>
          <div className="space-y-2.5">
            {topAds.map((ad, i) => {
              const st = getAdStatus(ad);
              return (
                <div key={ad.id} className={`flex items-center gap-3 py-2 ${i < topAds.length - 1 ? 'border-b border-stone-100' : ''}`}>
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${i === 0 ? 'bg-amber-100 text-amber-600' : 'bg-stone-100 text-stone-400'}`}>{i + 1}</span>
                  ...
                  <span className="text-sm font-bold text-stone-600">{formatMoney(ad.viewCount)}회</span>
                </div>
              );
            })}
          </div>
        ...
        {/* Expiring + Unpaid */}
        <div className="bg-white rounded-xl border border-stone-200 p-5">
          <h3 className="text-sm font-bold text-stone-700 mb-3">7일 내 만료 예정</h3>
          {expiring.length === 0 ? (
            <p className="text-sm text-stone-400 py-4 text-center">만료 임박한 광고가 없습니다</p>
          ) : (
            <div className="space-y-2.5">
              {expiring.map(ad => {
                const days = Math.ceil((new Date(ad.endDate).getTime() - Date.now()) / 86400000);
                return (
                  <div key={ad.id} className="flex items-center gap-3 py-2 border-b border-stone-100">
          ...
          <div className="mt-4 pt-3 border-t border-stone-100">
            <h4 className="text-xs font-bold text-stone-500 mb-2">미결제 항목</h4>
            {unpaid.length === 0 ? (
              <p className="text-xs text-stone-400">미결제 항목 없음</p>
            ) : unpaid.map((item, i) => (
...
const COLORS: Record<string, { bg: string; icon: string }> = {
  blue: { bg: 'bg-[#FDF2F2]', icon: 'text-[#7f2929]' },
...
const Card: React.FC<{ label: string; value: number; unit: string; sub: React.ReactNode; color: string }> = ({ label, value, unit, sub, color }) => {
  ...
  return (
    <div className="bg-white rounded-xl p-5 border border-stone-200 hover:shadow-md transition">
      <p className="text-xs font-medium text-stone-400 uppercase tracking-wider">{label}</p>
      <p className="text-3xl font-extrabold mt-1">{value}<span className="text-base font-medium text-stone-400 ml-1">{unit}</span></p>
