    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">유료 광고 · 기간별 이벤트/프로모션 관리</p>
        </div>
        {!readOnly && (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {ads.map(ad => {
                ...
                return (
                  <tr key={ad.id} className="border-b border-stone-100 hover:bg-[#FDF2F2]/30 transition">
                    <td className="px-4 py-3.5 font-semibold max-w-[200px] truncate">{ad.title}</td>
                    <td className="px-4 py-3.5 text-stone-500">{ad.advertiser}</td>
                    <td className="px-4 py-3.5"><span className={`text-xs font-semibold ${st.cls}`}>{st.label}</span></td>
                    <td className="px-4 py-3.5 text-stone-500 text-xs">{ad.period}</td>
                    ...
                    {!readOnly && (
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEdit(ad)} className="p-1.5 rounded-lg hover:bg-[#FDF2F2] text-stone-400 hover:text-[#7f2929] transition"><SquarePen className="w-4 h-4" /></button>
                          <button onClick={() => remove(ad.id)} className="p-1.5 rounded-lg hover:bg-red-100 text-stone-400 hover:text-red-500 transition"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </td>
                    )}
                  </tr>
      ...
      <Modal open={open} title={editing?.id ? '알짜광고 수정' : '알짜광고 등록'} onClose={close}>
        {editing && (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-stone-500 mb-1.5">광고 설명 *</label>
              <textarea value={editing.description} onChange={e => setEditing({ ...editing, description: e.target.value })} rows={3} className="w-full border border-stone-200 rounded-lg px-3 py-2.5 text-sm resize-none focus:ring-2 focus:ring-[#7f2929] focus:outline-none" />
            </div>
