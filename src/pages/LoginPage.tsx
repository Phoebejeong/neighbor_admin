      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className={`px-4 py-2.5 rounded-lg text-sm font-semibold whitespace-nowrap transition ${
            vf ? 'bg-emerald-100 text-emerald-500 cursor-default'
              : cs ? 'bg-stone-100 text-stone-600 hover:bg-stone-200'
...
export const LoginPage: React.FC<Props> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      ...
      toast('회원가입이 완료되었습니다! 가게를 등록해보세요');
      onLogin(userName, true, email);
    } else {
      if (!email.trim()) { toast('이메일을 입력해주세요', 'error'); return; }
      if (!password.trim()) { toast('비밀번호를 입력해주세요', 'error'); return; }
      // mock: 특정 이메일만 차단
      if (email === 'phoebe.jeong@sotatek.com') {
        toast('가입된 아이디가 없습니다', 'error'); return;
      }
  ...
  // PhoneVerifyBlock은 컴포넌트 밖으로 이동됨
    ...
    <div className="min-h-screen bg-stone-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <img src="/logo.png" alt="EYEVACS" className="w-16 h-16 rounded-xl object-cover mx-auto mb-3 cursor-pointer" onClick={() => setMode('login')} />
          <p className="text-stone-500 text-sm">우리동네 사장님 센터</p>
        </div>
          ...
          {mode === 'find' ? (
            <>
              <div className="px-6 pt-6 pb-2">
                <h3 className="text-lg font-bold text-stone-900">계정 찾기</h3>
                <p className="text-sm text-stone-500 mt-1">휴대폰 인증으로 아이디 또는 비밀번호를 찾습니다</p>
              </div>

              {/* 아이디/비밀번호 찾기 탭 */}
              <div className="flex mx-6 mt-3 bg-stone-100 rounded-lg p-1">
                <button
                  onClick={() => { setFindTab('id'); setFoundEmail(null); }}
                  className={`flex-1 py-2 rounded-md text-sm font-semibold transition ${findTab === 'id' ? 'bg-white text-[#7f2929] shadow-sm' : 'text-stone-500'}`}
                >
                  아이디 찾기
                </button>
                <button
                  onClick={() => { setFindTab('pw'); setPwResetDone(false); }}
                  className={`flex-1 py-2 rounded-md text-sm font-semibold transition ${findTab === 'pw' ? 'bg-white text-[#7f2929] shadow-sm' : 'text-stone-500'}`}
                >
                  비밀번호 찾기
                </button>
              </div>
                ...
                foundEmail ? (
                  <div className="p-6 text-center">
                    <div className="w-16 h-16 rounded-full bg-[#FDF2F2] flex items-center justify-center mx-auto mb-4">
                      <svg className="w-8 h-8 text-[#7f2929]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" /></svg>
                    </div>
                    <p className="text-sm font-semibold text-stone-700 mb-1">가입된 계정을 찾았습니다</p>
                    <div className="bg-stone-50 rounded-lg px-4 py-3 mt-3 mb-4">
                      <p className="text-lg font-bold text-stone-900">{foundEmail}</p>
                    </div>
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => { setFindTab('pw'); setFoundEmail(null); setPwResetDone(false); }} className="flex-1 py-2.5 rounded-lg border border-stone-200 text-sm font-medium text-stone-500 hover:bg-stone-50 transition">
                        비밀번호 찾기
                      </button>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleFindId} className="p-6 space-y-4">
                    ...
                    <button type="submit" disabled={!findIdVerified}
                      className="w-full py-3 rounded-lg bg-[#222] hover:bg-[#333] text-white text-sm font-semibold transition disabled:bg-stone-200 disabled:text-stone-400">
                      아이디 찾기
                    </button>
                    <button type="button" onClick={goLogin} className="w-full text-sm text-stone-500 hover:text-stone-700 transition">
                      로그인으로 돌아가기
                    </button>
                  </form>
                )
              ...
              {findTab === 'pw' && (
                pwResetDone ? (
                  <div className="p-6 text-center">
                    ...
                    <p className="text-sm font-semibold text-stone-700">비밀번호가 변경되었습니다</p>
                    <p className="text-xs text-stone-500 mt-1">새 비밀번호로 로그인해주세요.</p>
                  ...
                  <form onSubmit={handleFindPwReset} className="p-6 space-y-4">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-stone-500 mb-1.5">새 비밀번호 <span className="text-stone-300 font-normal">(8자 이상, 대/소문자, 숫자, 특수문자)</span></label>
                  ...
                  <form onSubmit={handleFindPwVerify} className="p-6 space-y-4">
                    <div>
                      ...
                      <input type="text" value={findPwEmail} onChange={e => setFindPwEmail(e.target.value)} placeholder="가입 시 사용한 이메일"
                        className="w-full border border-stone-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-[#7f2929] focus:outline-none" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-stone-500 mb-1.5">이름</label>
            ...
            <>
              <div className="px-6 pt-6 pb-2">
                <h3 className="text-lg font-bold text-stone-900">로그인</h3>
                <p className="text-sm text-stone-500 mt-1">이메일과 비밀번호를 입력하세요</p>
              </div>
                ...
                <div>
                  <label className="flex items-center gap-1.5 text-xs font-semibold text-stone-500 mb-1.5">
                    ...
                    <span className="relative group">
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5 text-stone-300 hover:text-[#7f2929] cursor-help transition" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M12 2a10 10 0 100 20 10 10 0 000-20z" /></svg>
                      <span className="absolute top-1/2 -translate-y-1/2 left-full ml-2 w-56 px-3 py-2 bg-stone-800 text-white text-xs rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition pointer-events-none text-center leading-relaxed shadow-lg z-50">
                        admin@gmail.com으로 로그인 시<br/>어드민 모드를 확인할 수 있습니다
                        <span className="absolute top-1/2 -translate-y-1/2 right-full border-4 border-transparent border-r-stone-800" />
                      </span>
                    </span>
                  </label>
              ...
              <div className="px-6 pb-5 space-y-2 text-center">
                <div className="flex items-center justify-center gap-2">
                  <button onClick={() => { setMode('find'); setFindTab('id'); }} className="text-xs text-stone-400 hover:text-[#7f2929] transition">
                    아이디 찾기
                  </button>
                  <span className="text-xs text-stone-300">|</span>
                  <button onClick={() => { setMode('find'); setFindTab('pw'); }} className="text-xs text-stone-400 hover:text-[#7f2929] transition">
                    비밀번호 찾기
                  </button>
                </div>
                <p className="text-xs text-stone-400">
                  아직 계정이 없으신가요?{' '}
                  <button onClick={() => setMode('signup')} className="text-[#7f2929] font-medium hover:underline">회원가입</button>
                </p>
              </div>
            </>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-lg shadow-sm w-full max-w-lg max-h-[80vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b border-stone-100 px-6 py-4 flex items-center justify-between">
                ...
                <button onClick={() => setShowTermsModal(null)} className="text-stone-400 hover:text-stone-600">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>
              <div className="p-6 text-xs text-stone-600 leading-relaxed space-y-3">
                {showTermsModal === 'terms' ? (
                  <>
                    ...
                    <p className="text-stone-400 mt-4">전문은 서비스 내 이용약관 페이지에서 확인할 수 있습니다.</p>
                  </>
                ) : (
                  <>
                    ...
                    <p className="text-stone-400 mt-4">전문은 서비스 내 개인정보처리방침 페이지에서 확인할 수 있습니다.</p>
                  </>
                )}
              </div>
              <div className="sticky bottom-0 bg-white border-t border-stone-100 px-6 py-3">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-lg shadow-sm w-full max-w-md max-h-[90vh] overflow-y-auto">
              <div className="px-6 pt-6 pb-2 flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-bold text-stone-900">회원가입</h3>
                  <p className="text-sm text-stone-500 mt-1">가게 정보를 등록하려면 먼저 가입하세요</p>
                </div>
                <button onClick={() => setMode('login')} className="text-stone-400 hover:text-stone-600 transition p-1">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-stone-500 mb-1.5">비밀번호 <span className="text-stone-300 font-normal">(8자 이상, 대/소문자, 숫자, 특수문자)</span></label>
                  <div className="relative">
                    <input type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••"
                      className="w-full border border-stone-200 rounded-lg px-3 py-2.5 pr-10 text-sm focus:ring-2 focus:ring-[#7f2929] focus:outline-none" />
                    <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600 transition">
                ...
                <div>
                  <label className="block text-xs font-semibold text-stone-500 mb-1.5">비밀번호 확인 *</label>
                  <div className="relative">
                    <input type={showPwConfirm ? 'text' : 'password'} value={passwordConfirm} onChange={e => setPasswordConfirm(e.target.value)} placeholder="비밀번호를 다시 입력해주세요"
                      className="w-full border border-stone-200 rounded-lg px-3 py-2.5 pr-10 text-sm focus:ring-2 focus:ring-[#7f2929] focus:outline-none" />
                    <button type="button" onClick={() => setShowPwConfirm(!showPwConfirm)} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600 transition">
                ...
                <div className="border border-stone-200 rounded-lg p-4 space-y-3">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      className="w-4 h-4 rounded border-stone-300 text-[#7f2929] focus:ring-[#7f2929]"
                    />
                    <span className="text-sm font-bold text-stone-900">전체 동의</span>
                  </label>
                  <div className="border-t border-stone-100 pt-3 space-y-2.5 ml-1">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={agreeTerms} onChange={e => setAgreeTerms(e.target.checked)}
                        className="w-3.5 h-3.5 rounded border-stone-300 text-[#7f2929] focus:ring-[#7f2929]" />
                      <span className="text-xs text-stone-600">[필수] 이용약관 동의</span>
                      <button type="button" onClick={() => setShowTermsModal('terms')} className="text-xs text-[#7f2929] hover:underline ml-auto">보기</button>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={agreePrivacy} onChange={e => setAgreePrivacy(e.target.checked)}
                        className="w-3.5 h-3.5 rounded border-stone-300 text-[#7f2929] focus:ring-[#7f2929]" />
                      <span className="text-xs text-stone-600">[필수] 개인정보 수집·이용 동의</span>
                      <button type="button" onClick={() => setShowTermsModal('privacy')} className="text-xs text-[#7f2929] hover:underline ml-auto">보기</button>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={agreeMarketing} onChange={e => setAgreeMarketing(e.target.checked)}
                        className="w-3.5 h-3.5 rounded border-stone-300 text-[#7f2929] focus:ring-[#7f2929]" />
                      <span className="text-xs text-stone-600">[선택] 마케팅 정보 수신 동의</span>
                    </label>
                  </div>
                </div>
