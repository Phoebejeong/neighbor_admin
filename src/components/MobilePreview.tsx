...
export const MobilePreview: React.FC<Props> = ({ title, children }) => (
  <div className="flex flex-col items-center">
    <p className="text-xs font-semibold text-gray-400 mb-2">앱에서 이렇게 보여요</p>
    <div className="w-[320px] border-[3px] border-gray-800 rounded-[2rem] overflow-hidden shadow-xl bg-white">
      ...
      {/* Content */}
      <div className="bg-gray-50 min-h-[200px] max-h-[400px] overflow-y-auto">
        {children}
      </div>
      {/* Bottom bar */}
      <div className="bg-white border-t border-gray-100 py-2 px-4 flex justify-around">
        {['홈', '걷기', '출입', '단지', '우리동네'].map(t => (
          <span key={t} className={`text-[9px] ${t === '우리동네' ? 'text-blue-600 font-bold' : 'text-gray-400'}`}>{t}</span>
        ))}
      </div>
    </div>
  </div>
    ...
    <div className="flex items-center gap-2.5">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[9px] text-gray-400">{category}</span>
        </div>
        <p className="text-[10px] text-gray-500 truncate mt-0.5">{tagline || '홍보문구를 입력해주세요'}</p>
        {address && <p className="text-[9px] text-gray-400 mt-0.5">📍 {address}</p>}
      </div>
      {phone && (
  ...
  <div className="mx-3 my-1.5 bg-white rounded-xl p-3 shadow-sm">
    <div className="flex gap-2.5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-gray-500 truncate mt-0.5">{description || '광고 내용'}</p>
        <div className="flex items-center gap-1.5 mt-1">
          <span className="text-[8px] px-1.5 py-0.5 rounded-full bg-green-100 text-green-700 font-bold">진행중</span>
          <span className="text-[9px] text-gray-400">📅 {period || '기간 미정'}</span>
        </div>
      </div>
    </div>
  </div>
...
export const ShoppingPreviewCard: React.FC<{ name: string; price: number; discountRate: number; shopName: string; image?: string }> = ({ name, price, discountRate, shopName, image }) => {
  ...
  return (
    <div className="bg-white rounded-xl p-2.5 shadow-sm">
      <div className="w-full h-16 rounded-lg bg-gray-100 flex items-center justify-center overflow-hidden mb-1.5">
        {image ? <img src={image} alt="" className="w-full h-full object-cover" /> : <span className="text-gray-300 text-lg">🛒</span>}
      </div>
      ...
      <p className="text-[8px] text-gray-400 mt-0.5">{shopName}</p>
    </div>
  );
};
